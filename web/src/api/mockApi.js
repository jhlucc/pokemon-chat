...
function mockAgentStream({ agent_name, query, meta } = {}) {
  ...
  return ndjsonResponse([
    { request_id: requestId, response: '', status: 'init', meta: { server_model_name: 'mock/offline' }, msg: { id: msgId, type: 'assistant' } },
    ...chunks.map((part) => ({
...
export function resolveMockResponse(path, { method = 'GET', query, body } = {}) {
  ...
  if (m === 'POST' && p === '/tools/file-chunking') {
    ...
    const chunkOverlap = body?.chunk_overlap ?? body?.params?.chunk_overlap ?? body?.params?.chunkOverlap ?? 20;
    const input = body?.text ?? body?.file ?? '';
    const nodes = chunkPlainText(input, { chunkSize, chunkOverlap }).map((c) => ({ text: c.text, meta: c.meta }));
  ...
  if (m === 'GET' && p === '/data/graph/nodes') return jsonResponse(mockGraphSampleNodes({ num: query?.num }));
  if (m === 'GET' && p === '/data/graph/node') return jsonResponse(mockGraphSearch({ entity_name: query?.entity_name }));
