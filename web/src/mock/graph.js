...
export function mockGraphSampleNodes({ num } = {}) {
  ...
  const edges = cloneEdges((demoGraph?.edges || []).filter((e) => allowed.has(String(e.source_id)) && allowed.has(String(e.target_id))));
...
export function mockGraphSearch({ entity_name } = {}) {
  ...
  const edges = cloneEdges((demoGraph?.edges || []).filter((e) => neighborIds.has(String(e.source_id)) && neighborIds.has(String(e.target_id))));
  return { result: { nodes, edges } };
}
