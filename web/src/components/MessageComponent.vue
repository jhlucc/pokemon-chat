...
/* ===== 公共文字 / loading / 提示 ===== */
.retry-hint{margin-top:8px;padding:8px 16px;color:var(--chat-muted-text);font-size:14px;text-align:left;}
.retry-link{color:var(--main-600);cursor:pointer;margin-left:4px;&:hover{text-decoration:underline;}}
.ant-btn-icon-only:has(.anticon-stop){background: var(--danger-600) !important;&:hover{background:var(--danger-500)!important;}}
.loading-dots{display:inline-flex;align-items:center;justify-content:center;
  div{width:8px;height:8px;margin:0 4px;background:var(--chat-muted-text);border-radius:50%;opacity:.3;animation:pulse .5s infinite both;
...
/* ===== message-box 内排版 ===== */
.message-box{
  display:inline-block;border-radius:var(--radius-lg);margin:.8rem 0;padding:.625rem 1.25rem;
  user-select:text;word-break:break-word;font-size:15px;line-height:24px;max-width:100%;position:relative;
  &.assistant,&.received{color:var(--chat-assistant-text);width:100%;text-align:left;margin:0 0 16px;padding:0;background:transparent}
  .err-msg{
  ...
  .searching-msg{color:var(--chat-muted-text);animation:opacityPulse 1s infinite ease-in-out}
  .reasoning-box{margin:10px 0 15px;border:1px solid var(--border-color);border-radius:var(--radius-sm);
    .reasoning-content{font-size:13px;color:var(--chat-muted-text);white-space:pre-wrap;margin:0}}
...
@keyframes opacityPulse{0%{opacity:.7}50%{opacity:1}100%{opacity:.7}}
...
:deep(.tool-call-display){
  ...
  /* 小图标配色 */
  .anticon{color:var(--gray-600);cursor:pointer;&:hover{color:var(--gray-800)}}
}
...
.message-md .md-editor-preview-wrapper{
  ...
  a:hover{color:var(--main-400); text-decoration: underline;}
  code{
    font-size:13px;
