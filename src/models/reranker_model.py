...
class SiliconFlowReranker(BaseReranker):
    ...
    def rerank(self, query: str, documents: list[str], top_k: int = 10) -> list[tuple[int, float, str]]:
        ...
        return [(r["index"], r["relevance_score"], documents[r["index"]]) for r in result["results"]]
...
class DashScopeReranker(BaseReranker):
    ...
    def rerank(self, query: str, documents: list[str], top_k: int = 10) -> list[tuple[int, float, str]]:
        ...
        return [(r["index"], r["relevance_score"], documents[r["index"]]) for r in result["output"]["results"]]
...
class JinaReranker(BaseReranker):
    ...
    def rerank(self, query: str, documents: list[str], top_k: int = 10) -> list[tuple[int, float, str]]:
        headers = {"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"}
        payload = {"model": self.model, "query": query, "documents": documents, "top_n": top_k}
...
def get_reranker(provider: str = None, model: str = None, api_key: str = None) -> BaseReranker:
    """
    获取 Reranker 实例
