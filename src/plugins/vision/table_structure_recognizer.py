    ...
    def __init__(self):
        try:
            ...
            super().__init__(
                ...
                "tsr",
                snapshot_download(
                    repo_id="InfiniFlow/deepdoc", local_dir=get_default_resource_dir(), local_dir_use_symlinks=False
                ),
            )
            ...
            while j < len(tbl[0]):
                ...
                if left < right:
                    ...
                    for i in range(len(tbl)):
                        tbl[i].pop(j)
                cols.pop(j)
        assert len(cols) == len(tbl[0]), f"Column NO. miss matched: {len(cols)} vs {len(tbl[0])}"
