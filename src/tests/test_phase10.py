...
class TestCRAGEvaluator:
    ...
    def test_grade_no_docs(self):
        with patch("src.graph.nodes.crag.settings"):
            from src.graph.nodes.crag import CRAGEvaluator
...
class TestQueryDecomposer:
    ...
    def test_is_complex_detection(self):
        with patch("src.knowledge.core.query_decomposer.settings"):
            ...
            assert decomposer.is_complex("Compare Pikachu and Charmander") is True
            assert decomposer.is_complex("What is the difference between fire and water?") is True
            assert decomposer.is_complex("What's the difference between fire and water types?") is True

            # Simple queries
            assert decomposer.is_complex("Tell me about Pikachu") is False
            assert decomposer.is_complex("What is Pikachu's type?") is False
