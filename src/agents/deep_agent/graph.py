...
POKEMON_KNOWLEDGE = {
    "皮卡丘": {
        ...
        "type": "电", "generation": 1, "category": "鼠宝可梦"
    },
    "喷火龙": {
        "facts": ["火/飞行属性", "小火龙的最终进化", "可mega进化为X和Y两种形态"],
        "type": "火/飞行", "generation": 1, "category": "火焰宝可梦"
    },
    "超梦": {
        "facts": ["由梦幻基因改造而来", "超能力属性", "种族值680是准神级别"],
        "type": "超能力", "generation": 1, "category": "基因宝可梦"
    },
    "快龙": {
        "facts": ["龙/飞行属性", "第一世代准神", "可学习神速招式"],
        "type": "龙/飞行", "generation": 1, "category": "龙宝可梦"
    },
}
...
async def process_learnings_node(state: DeepContext, config: RunnableConfig) -> Dict[str, Any]:
    ...
    # 生成后续研究问题
    follow_up = []
    if any("进化" in l for l in learnings):
        follow_up.append("进化条件研究")
    if any("属性" in l for l in learnings):
        follow_up.append("属性克制深入分析")
    if any("招式" in l for l in learnings):
...
"""
    ...
    return {
        "final_report": report,
        "messages": [AIMessage(content="研究报告生成完成")]
    }
...
class DeepAgent(BaseAgent):
    ...
    def get_info(self) -> dict:
        return {
            "name": "DeepAgent",
            "description": "Pokemon 深度研究代理 - 迭代式多轮研究，自动生成研究报告"
        }
