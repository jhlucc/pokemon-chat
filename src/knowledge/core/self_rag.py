...
class RetrievalDecision(BaseModel):
    ...
    query_type: Literal["factual", "creative", "conversational", "complex"] = Field(
...
class SupportCheck(BaseModel):
    ...
    support_level: Literal["full", "partial", "none"] = Field(
        description="Level of support from docs"
    )
...
RETRIEVAL_DECISION_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """你是宝可梦知识检索决策系统。判断用户的问题是否需要检索宝可梦知识库。
...
RELEVANCE_CHECK_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """你是宝可梦知识相关性评估员。评估检索到的文档是否与用户问题相关。
...
SUPPORT_CHECK_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """你是宝可梦知识事实核查员。检查生成的回答是否有文档支持。
...
class SelfRAG:
    ...
    def should_retrieve(self, query: str) -> RetrievalDecision:
        ...
        try:
            ...
            return RetrievalDecision(
                ...
                reason=f"Default to retrieve due to error: {e}",
                query_type="factual"
            )
    ...
    def check_support(self, query: str, documents: str, answer: str) -> SupportCheck:
        ...
        """
        try:
            check = self.support_chain.invoke({
                "query": query,
            ...
            return SupportCheck(
                ...
                unsupported_claims=f"Check failed: {e}"
            )
