...
DRAFT_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """你是宝可梦知识草案生成器。根据用户问题和参考资料，生成一个草案回答。
...
VERIFY_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """你是宝可梦知识专家审核员。给定多个草案回答，选择最佳的一个并可选择性地优化它。
...
class SpeculativeRAG:
    ...
    def __init__(self, drafter_model: str | None = None, verifier_model: str | None = None, num_drafts: int = 3):
    ...
    def generate_drafts(self, query: str, context: str) -> list[DraftAnswer]:
        ...
        with ThreadPoolExecutor(max_workers=self.num_drafts) as executor:
            futures = [
                executor.submit(self.draft_chain.invoke, {"query": query, "context": context})
    ...
    def verify_and_select(self, query: str, context: str, drafts: list[DraftAnswer]) -> VerificationResult:
        ...
        try:
            result = self.verify_chain.invoke({"query": query, "context": context, "drafts": drafts_text})
            return result
        except Exception as e:
